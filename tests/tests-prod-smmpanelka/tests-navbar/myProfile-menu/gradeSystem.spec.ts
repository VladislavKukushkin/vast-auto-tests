...
test('test', async ({ page }) => {
    ...
    await page.locator('#app > div.v-application--wrap > div.fill-height.position-relative > div > header > div.header__options > div:nth-child(2) > button.btn.header__profile.header__options--item.header__profile--logged')
        .click();
    await page.locator('#app > div > div.fill-height.position-relative > div > div > div.profile__wrapper > div > ul > button:nth-child(4) > a')
        .click();
    await page.waitForTimeout(1000);
    await expect (page).toHaveURL('https://smmpanelka.com/app/grades');
    console.log('The grades system menu is available')
