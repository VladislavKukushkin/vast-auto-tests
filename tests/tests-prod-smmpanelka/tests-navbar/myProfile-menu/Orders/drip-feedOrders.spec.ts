...
test('test', async ({ page }) => {
    ...
    await page.locator('#app > div.v-application--wrap > div.fill-height.position-relative > div > div > div.profile__wrapper > div > ul > div:nth-child(3) > button')
        .click();
    await page.locator('#app > div > div.fill-height.position-relative > div > div > div.profile__wrapper > div > ul > div.drop-down.nav__item.drop-down.opened > div > div:nth-child(3) > button > a')
        .click()
    await page.waitForTimeout(1000);
    await expect (page).toHaveURL('https://smmpanelka.com/app/drip-feed');
    console.log('Drip-feed menu is available')
